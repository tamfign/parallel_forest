...
void ArffImporter::BuildInstanceTable()
{
	...
	for (unsigned int i = 0; i < numInstances; i++)
	{
		...
		memmove(offset,
				instanceVec[i].featureAttrArray, numFeatures * sizeof(double));
	...
	while (fgets(buffer, READ_LINE_MAX, fp) != nullptr)
	{
		...
		if (StrEqualCaseInsen(firstToken, KEYWORD_ATTRIBUTE))
		{
			...
			else
			{
				...
				while (sscanf(featureType, "%[^,}]%n", className, &readSize) >
					   0)
				{
		...
		else if (StrEqualCaseInsen(firstToken, KEYWORD_DATA))
		{
			...
			unsigned int featureAttrArraySize = numFeatures * sizeof(double);
			...
			while (fgets(buffer, READ_LINE_MAX, fp) != nullptr)
			{
				...
				instance.featureAttrArray =
					(double *) malloc(featureAttrArraySize);
